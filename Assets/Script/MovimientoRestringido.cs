...
public class MovimientoRestringido : MonoBehaviour
{
    // Agacharse
    public bool crouch;
    ...
    // Mecanica de las Pinzas
    public float reducciónVelocidad = 1.5f;
    public GameObject Mano;

    public Pinzas ManoReferenece;
    bool heavyObjectPicked;
    ...
    void FixedUpdate()
    {
        ...
        ManoReferenece = Mano.GetComponent<Pinzas>();
        heavyObjectPicked = ManoReferenece.GetpickedHVYobject();
    ...
    void Update()
    {
        ...
        {
            if (heavyObjectPicked == true)
            {
                velocidadMovimiento = velCorrer - reducciónVelocidad; 
            }
            else
            {
        ...
        else if (crouch) // Reducir la velocidad si está agachado
        {
            if (heavyObjectPicked == true)
            {
                velocidadMovimiento = velocidadAgachado - reducciónVelocidad;
            }
            else    
            {
        ...
        else
        {
            if (heavyObjectPicked == true)
            {
                velocidadMovimiento = velocidadBase - reducciónVelocidad; // Restaurar la velocidad base
            }
            else
            {
